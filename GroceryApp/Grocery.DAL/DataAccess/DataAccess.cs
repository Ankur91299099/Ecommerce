    ...
    public class DataAccess : IDataAccess
    {
        ...
        public bool InsertUser(User user)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                query = "INSERT INTO Users (FirstName, LastName, Address, Mobile, Email, Password, CreatedAt, ModifiedAt,Role) values (@fn, @ln, @add, @mb, @em, @pwd, @cat, @mat,@rol);";
                ...
                command.Parameters.Add("@rol", System.Data.SqlDbType.NVarChar).Value = "user";
        ...
        public bool newProduct(AddProduct product)
        {
            ...
            string query = "INSERT INTO Products (Title, Description, CategoryId, OfferId, Price, Quantity, ImageName) VALUES (@ti, @des, @cid, @oid, @pr, @qt, @img);";
            command.CommandText = query;
           // command.Parameters.Add("@pid", System.Data.SqlDbType.Int).Value = product.ProductId;
            command.Parameters.Add("@ti", System.Data.SqlDbType.NVarChar).Value = product.Title;
            command.Parameters.Add("@des", System.Data.SqlDbType.NVarChar).Value = product.Description;
            command.Parameters.Add("@cid", System.Data.SqlDbType.Int).Value = product.CategoryId;
            command.Parameters.Add("@oid", System.Data.SqlDbType.Int).Value = product.OfferId;
            command.Parameters.Add("@pr", System.Data.SqlDbType.Int).Value = product.Price;
            command.Parameters.Add("@qt", System.Data.SqlDbType.Int).Value = product.Quantity;
            command.Parameters.Add("@img", System.Data.SqlDbType.NVarChar).Value = product.ImageName;
        ...
        public bool DeleteCartItem(int userId, int productId)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                query = "SELECT CartItemId FROM CartItems WHERE CartId=" + cartId + " AND ProductId=" + productId + "";
                command.CommandText = query;
        ...
        public void UpdateProduct(int id, AddProduct updatedProduct)
        {
            using (var connection = new SqlConnection(dbconnection))
            {
                connection.Open();
                var query = @"UPDATE Products SET Title = @Name,Description = @Description, Price = @Price,Quantity = @Quantity, ImageName = @ImageName  WHERE ProductId = @ProductId";
                var command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@Name", updatedProduct.Title);
                ...
                //command.Parameters.AddWithValue("@OfferId", updatedProduct.OfferId);
                command.Parameters.AddWithValue("@Quantity", updatedProduct.Quantity);
                command.Parameters.AddWithValue("@ImageName", updatedProduct.ImageName);
                command.ExecuteNonQuery();
                connection.Close();
            }
        }
